package com.mygdx.game.models.asteroids;
...
public class Asteroid extends Sprite{
    ...
    private Sound crush = Gdx.audio.newSound(Gdx.files.internal("crush.wav"));

    public Asteroid(TextureAtlas atlas, TextureAtlas explosion, TextureRegion region, float positionX, float positionY, float speedX, float speedY, float size, float speedRorate, float rorate) {
        super(region);
        this.atlas = atlas;
    ...
    public void draw(SpriteBatch batch, float delta, Hero hero) {
        if (whole) {
            batch.draw(
                    runningAnimation.getKeyFrame(elapsedTime, true), // текущий регион
            ...
            if (!stopAnimation) {
                batch.draw(
                        explosionAnimation.getKeyFrame(elapsedExplosionTime, true), // текущий регион
                        getLeft(), getTop(), // точка отрисовки
                        halfWidth, halfHeight, // точка вращения
                        getWidth(), getHeight(), // ширина и высота
